...
# def DOS_region(DOS_bounds, grid):
        
#     DOS_ith_grid = np.zeros([DOS_bounds.shape[0], grid])
#     meshgrid = np.zeros(grid)
#     for i in range(DOS_bounds.shape[0]):
#         DOS_ith_grid[i,:] = np.linspace(DOS_bounds[i,0],DOS_bounds[i,1], grid)
#         print(i)
            
#     # for j in range(DOS_bounds.shape[0]):
#     #     meshgrid[i,:] =  np.meshgrid(DOS_ith_grid[:,i])
    
#     dim =  DOS_bounds.shape[0]
#     # np.mgrid(*DOS_ith_grid)
#     # teste = np.meshgrid([DOS_ith_grid])
#     teste = ndmesh(*DOS_ith_grid)
#     # DOS_Y1 = np.linspace(DOS[:,0],DOS[:,1], DOS_resolution)
#     # DOS_Y2 = np.linspace(35,45, DOS_resolution)
#     # DOS_Y1, DOS_Y2 = np.meshgrid(DOS_Y1, DOS_Y2)
#     # DOS_Y1 = DOS_Y1.flatten()
#     # DOS_Y2 = DOS_Y2.flatten()
#     # DOS_points =  np.c_[DOS_Y1, DOS_Y2]
#     DOS_pts =1
...
def nlp_based_approach(DOS_bounds: np.ndarray, 
                       ...
                       ub: np.ndarray,
                       constr=None,
                       method: str ='DE', plot=True, ad=False) -> Union[np.ndarray,np.ndarray,list]:
    ...
    if fDIS.shape[1] > 2:
        ...
        if plot is True:
           ...
           # plt.rcParams['font.size'] = 16
            ...
            plt.savefig("inverse mapping.png")
            plt.show
